...
void pe_vk_semaphores_create(){
  ...
  for (uint32_t i = 0; i < PE_VK_FRAMES_IN_FLIGHT; i++) {
    vkCreateSemaphore(vk_device, &info, NULL,
                      &pe_vk_semaphore_images_available[i]);
    vkCreateFence(vk_device, &fence_info, NULL, &pe_vk_fence_in_flight[i]);
  }
  ...
  for (uint32_t i = 0; i < pe_vk_swapchain_image_count; i++)
    vkCreateSemaphore(vk_device, &info, NULL,
                      &pe_vk_semaphore_render_finished[i]);

  ZERO(pe_vk_fence_image_in_flight);

  pe_vk_current_frame = 0;
}

void pe_vk_end_sync(){

  for (uint32_t i = 0; i < PE_VK_FRAMES_IN_FLIGHT; i++) {
    vkDestroySemaphore(vk_device, pe_vk_semaphore_images_available[i], NULL);
    vkDestroyFence(vk_device, pe_vk_fence_in_flight[i], NULL);
  }

  for (uint32_t i = 0; i < pe_vk_swapchain_image_count; i++)
    vkDestroySemaphore(vk_device, pe_vk_semaphore_render_finished[i], NULL);
}
